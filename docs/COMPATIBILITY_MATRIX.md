...
| Quick screen and USB | SBS source remains exact | Activity state recognized | Firmware-controlled quick-screen projection; stock USB boundary | Quick-screen simulation in SBS viewport | Simulation state only | Source paths inspected | No `.qs` or `.usb` source file is invented. |
| Rockbox `.fnt` assets | Exact binary bytes | RB12 header and metrics | Height, ascent, maximum width, range, glyph count | Browser glyph raster remains approximate | Existing `.fnt` import and package export | Generated font loaded in current Rockbox simulator | TTF/OTF conversion is external pending a browser delivery/licensing decision. |
