...
## ADR-0020 — Keep asset bytes canonical and mutate only resolved references

**Status:** Accepted

**Context:** Imported ZIP assets already had safe archive-path identity, but the ordinary upload controls still treated data URLs and basenames as editable state. That cannot distinguish duplicate filenames in different screen directories, inspect real Rockbox BMP compatibility, or rename an asset without risking unrelated source. Rockbox also permits the compact `%xl(handle,file,frames)` form used throughout Adwaitapod.

**Decision:** `ThemePackage.assets`, `ProjectState.projectAssets`, and `ProjectState.componentAssets` are the canonical byte stores. Preview object URLs and decoded browser images are disposable projections. The Assets workspace resolves known WPS/SBS/FMS and CFG references by exact archive path and the source screen's bitmap directory. Replacement preserves the path and source; rename rewrites only references proven to resolve to that exact path; delete refuses resolved or component-owned assets. Unknown and future source stays untouched and therefore cannot be assumed safe to rewrite.

BMP inspection follows the formats accepted by the pinned Rockbox loader. PNG/JPEG inputs are decoded locally and converted to deterministic 24-bit BMP, or 32-bit alpha bitfields when alpha is required. Vertical strips contain equal-size frames stacked top-to-bottom, matching `%xl` subimage handling. License-clean starter assets are generated locally and never insert source implicitly. Assets is lazy-loaded so the conversion/workshop UI does not enlarge the initial editor path.

**Consequences:** Imported bytes round-trip unchanged until the user explicitly replaces or renames them; duplicate basenames remain safe; project-added bytes export deterministically; and Adwaitapod's compact 12-frame battery strips can be inspected frame by frame. Unsupported reference-bearing syntax remains preserved but may not be discovered by the safety console, so “no known references” is not a blanket deletion guarantee. Browser bitmap display remains an editor preview; the external pinned Rockbox simulator is still the Level C rendering authority.

## ADR-0021 — Decode canonical RB12 fonts; catalog the separate Rockbox font pack without bundling it

**Status:** Accepted

**Context:** The Phase 3 import modal could validate a header, package exact FNT bytes, or invoke the local helper, but it could not browse project fonts, draw their glyphs, compare widths, detect range misses, or safely manage references. Pulp calls for a dedicated Font mode. Rockbox source also distinguishes loadable theme FNT files from its separately downloadable collection of BDF-derived fonts, whose files have individual redistribution licenses.

**Decision:** Make exact package bytes the only font asset authority. Independently validate the complete RB12 layout and decode its two evidenced pixel organizations: rotated 1-bit BDF blocks from `convbdf` and packed 4-bit antialias pixels from `convttf`. Derive glyph canvases, stored advance measurement, line height, declared-range misses, probable aliases of the default glyph, and comparison views from those bytes. Selecting a UI font updates the final `font:` CFG setting with minimum change; rename and delete use the exact-path asset reference graph for CFG and `%Fl`.
...
## ADR-0023 — Commit project-wide Theme settings through one canonical CFG transaction

**Status:** Accepted

**Context:** Project-wide controls were scattered across a settings panel and a generated, read-only Source CFG tab. Imported CFG bytes were preserved by the package layer, but users could not safely edit the whole document, metadata had no explicit home, editor-created projects had no canonical CFG after a source edit, and replacing the whole `ProjectState` caused avoidable preview work. Pulp calls for a dedicated Theme mode without turning imported source into visual elements.

**Decision:** Add `rockbox/theme/` as the single Theme/CFG transaction layer. Imported projects continue to own their document in `ThemePackage.cfg`. Editor-created projects receive a mutually exclusive `ProjectState.standaloneThemeConfig` only on their first real commit. Typed controls update only the final matching verified CFG key; raw edits remain exact and then re-project the known settings subset. Author and description are project metadata, never guessed CFG names or injected comments.

Verify the exposed names, enumerations, and numeric ranges against pinned Rockbox `apps/settings_list.c`. Reject unsafe CFG/screen archive paths and relocate an existing canonical screen document when its final CFG reference changes. Preserve unsupported settings, malformed lines, duplicates, comments, whitespace, and line endings. Source CFG Apply must call the same transaction as the lazy Theme workspace.

Keep the Theme draft outside `ProjectState` until **Commit project**. Preserve asset, screen, and render-setting identities for metadata or unknown-only edits, and make the canvas paint effect depend only on render-relevant state. Represent all three standard status-bar positions (`off`, `top`, `bottom`) distinctly; the legacy synthetic renderer may approximate the selected edge without making a firmware-pixel claim.

**Consequences:** Imported and editor-created themes now have a real editable CFG authority that survives JSON persistence and deterministic ZIP export. Comments never appear as elements; unknown future syntax remains openable; duplicate keys preserve Rockbox's final-value behavior; screen-path edits cannot silently disconnect an exported source document. Config-only work adds history and package state without repainting the preview. The complete Rockbox settings catalog, exact firmware menus, general font rasterization, and final device behavior remain outside the typed subset and external Level C authority.
