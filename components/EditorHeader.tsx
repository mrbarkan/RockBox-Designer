...
const TabButton = ({ id, label, activeId, onClick }: { id: ScreenType, label: string, activeId: ScreenType, onClick: (id: ScreenType) => void }) => (
    <button onClick={() => onClick(id)} className={`px-3 py-2 text-xs font-bold rounded-sm uppercase ${activeId === id ? 'bg-white shadow-sm text-black' : 'text-gray-500 hover:text-black'}`}>{label}</button>
);
...
export const EditorHeader: React.FC<EditorHeaderProps> = ({
    ...
    return (
        <div className="h-14 metal-gradient flex items-center px-3 justify-between border-b border-black select-none">
